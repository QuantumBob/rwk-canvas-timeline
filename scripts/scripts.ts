        ...
        if (tFile != null) {
            ...
            await this.app.fileManager.processFrontMatter(tFile, (frontmatter: any) => {
                if(!('pov' in frontmatter)) {
                    ...
                    Object.assign(card, {'pov': frontmatter['pov']})
                }
                if(!('participants' in frontmatter)) {
                    frontmatter['participants'] = 'none';
                } else {
                    Object.assign(card, {'participants': frontmatter['participants']})
                }
                if(!('date' in frontmatter)) {
                    frontmatter['date'] = getDate().toString();
                } else {
                    Object.assign(card, {'date': frontmatter['date']})
                }
            })
            if(!('basename' in card)) {
