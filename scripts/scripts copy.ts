...
function processGroup (plugin: RwkCanvasTimelinePlugin,cards: any, groups: any) {
    ...
    // for (const group of groups) {
        groups.forEach((group: any) => {
            ...
            groupedCards.sort((a:any, b:any) => { 
                if (a.y - b.y == 0) {
                    return a.x - b.x;
                } else return a.y - b.y;
            });
    
            groupedCards.forEach((data: any) => {
                // load each card in the group
                const tFile = plugin.app.vault.getFileByPath(data.file);
                if (tFile != null) {
                    // check if the page the card comes from has these in its frontmatter
                    this.app.fileManager.processFrontMatter(tFile, (frontmatter: any) => {
                        if(!('pov' in frontmatter)) {
                            frontmatter['pov'] = 'none';
                        } else {
                            Object.assign(data, {'pov': frontmatter['pov']})
                        }
                        if(!('participants' in frontmatter)) {
                            frontmatter['participants'] = 'none';
                        } else {
                            Object.assign(data, {'participants': frontmatter['participants']})
                        }
                    })
                    if(!('baseName' in data)) {
                        data['basename'] = 'none';
                    } else {
                        Object.assign(data, {'basename': tFile.basename})
                    }
                }
                if(Object.hasOwn(group, "label")) {
                    Object.assign(data, {"group-label": group.label.toLowerCase()});
                } else {
                    Object.assign(data, {"group-label": "unknown"});
                }
                rows.push(data);
            });
        });
...
function createMarkdownTable(plugin: RwkCanvasTimelinePlugin, rows: any) {
   // Now create markdown table    
    ...
    const tableSep = '|';
    //const space = " ";
    const tableHeading = '|Title|POV|Date|\n|---|---|---|\n'
    const output = rows.map((data: { basename: string; pov: string; date: string; }) => {
    // const output =  await Promise.all(rows.map(async (data: { basename: string; pov: string; date: string; }) => {
        // return [povClassStart + data.pov + povClassEnd + dv.fileLink(data.file) + spanEnd, data.pov, data.date, numPages.toFixed(2)];
        return [tableSep + '[[' + data.basename + ']]', tableSep + data.pov, tableSep + data.date];
    });
    console.log('RWK :' + output.join('|\n'));
...
async function getJSONObject(plugin: RwkCanvasTimelinePlugin) {
    ...
    if (tFile == null) {
        ...
        jsonString = await plugin.app.vault.read(tFile);
    }
...
function getWordCount(text: string): number {
    ...
    const pattern = new RegExp([
        ...
        nonSpaceDelimitedWordsOther,].join("|"),"g");
